...
impl <'a, T: web3::Transport> Wallet <'a, T> {
  ...
  pub fn guardians(&self) -> Result<Vec<Address>, String> {
    let guardian_manager = Address::from_str(&"FF5A7299ff6f0fbAad9b38906b77d08c0FBdc9A7").unwrap();
    let guardian_manager = Contract::from_json(self.web3.eth(), guardian_manager, constants::abis::GUARDIAN_MANAGER).unwrap();
    let result = guardian_manager.query("guardianStorage", (), None, Options::default(), None);    
    
    let guardian_storage = match result.wait() {
      Ok(s) => s,
      Err(_e) => return Err(format!("unable to fetch guardian storage address for {:?}", self.address)),
    };
    let guardian_storage = Contract::from_json(self.web3.eth(), guardian_storage, constants::abis::GUARDIAN_STORAGE).unwrap();
    let result = guardian_storage.query("getGuardians", (self.address,), None, Options::default(), None);
  ...
  pub fn modules(&self) -> Result<Vec<Address>, String> {
    ...
    let filter = FilterBuilder::default()
      ...
      .topics(
        Some(vec!["8da3ff870ae294081392139550e167f1f31f277f22015ee22fbffdbd7758f4e1".parse().unwrap()]),
        None,
        None,
        None,
      )
    ...
    let logs = match result.wait() {
      Ok(s) => s,
      Err(_e) => return Err(format!("unable to fetch modules logs for {:?}", self.address)),
    };
